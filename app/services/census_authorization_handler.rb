...
# Checks the authorization against the census for Pamplona.
require "digest/md5"
...
class CensusAuthorizationHandler < Decidim::AuthorizationHandler
  ...
  def document_type_valid
    ...
    if respuesta == false
      case codiError
        when 0
          errors.add(:handler_name, I18n.t("census_authorization_handler.invalid_something"))
        when 1
          errors.add(:document_number, I18n.t("census_authorization_handler.invalid_document"))
        when 2
          errors.add(:postal_code, I18n.t("census_authorization_handler.invalid_postal_code"))
        when 3
          errors.add(:date_of_birth, I18n.t("census_authorization_handler.invalid_date_of_birth"))
      end

    elsif respuesta != "true"
      return nil
    end
  ...
  def response
    ...
    response ||= Faraday.post Rails.application.secrets.census_url, request_body
