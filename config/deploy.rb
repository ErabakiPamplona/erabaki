...
namespace :puma do
  ...
  task :restart do
    on roles(:app)do
      execute "cd #{current_path} && PUMA_BIND='#{fetch(:puma_bind)}' /usr/local/rvm/bin/rvm default do bundle exec puma -C #{current_path}/config/puma.rb"
    end
  end
...
namespace :deploy do
  ...
  task :check_revision do
    on roles(:app) do
      ...
      if branch.match?(/\A[0-9a-f]{7,40}\z/)
        puts "WARNING: Skipping revision check for SHA branch #{branch}"
        next
      end

      local_head = `git rev-parse HEAD`.strip
      remote_head = `git rev-parse origin/#{branch}`.strip
      unless local_head == remote_head
        puts "WARNING: HEAD is not the same as origin/#{branch}"
        puts "Run `git push -u origin #{branch}` to sync changes."
        exit
      end
    end
  end
